...
The four SVGs in `experience/` embed the original vector artwork alongside the roles and dates supplied in the reference. These variants use blue logos (`#79b8ff` on dark backgrounds, `#2463a6` on light backgrounds), centered captions, and transparent backgrounds. They contain no external images, fonts, or scripts. The README selects a variant using GitHub-supported `<picture>` elements.

To change a role or date, update the `<title>`, `<desc>`, and caption text in both theme variants and the corresponding image's `alt` text in the main README.
