...
The four SVGs in `experience/` contain only the company logos. These variants use blue artwork (`#79b8ff` on dark backgrounds, `#2463a6` on light backgrounds), centered positioning, and transparent backgrounds. They contain no external images, fonts, or scripts. The README selects a variant using GitHub-supported `<picture>` elements.

When replacing a company logo, update both theme variants and the corresponding image's `alt` text in the main README.
